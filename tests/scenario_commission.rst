...
Install account_invoice_discount and commission::
    ...
    >>> account_invoice_module, commission_module = Module.find(
    ...     [('name', 'in', ['account_invoice_discount', 'commission'])])
    >>> Module.install([account_invoice_module.id, commission_module.id], config.context)
...
Create tax::
    ...
    >>> tax.save()
    >>> tax2 = set_tax_code(create_tax(Decimal('.10')))
    >>> tax2.save()
...
Create product sold::
    ...
    >>> template.account_expense = expense
    >>> template.account_revenue = revenue
    >>> template.customer_taxes.append(tax2)
